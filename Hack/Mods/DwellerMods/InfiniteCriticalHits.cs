...
{
    class InfiniteCriticalHits : Module
    {
        public InfiniteCriticalHits() : base("Infinite Criticals", KeyCode.None, Categories.mDweller, GuiNames.Toggle)
        {
        }
            ...
            if (MonoSingleton<DwellerManager>.IsInstanceValid)
                foreach (Dweller inhabitant in MonoSingleton<DwellerManager>.Instance.Dwellers)
                {
                    if (!inhabitant.IsRaider)
                        inhabitant.IncreaseCriticalHitMeter();
                }
        }
    }
}
